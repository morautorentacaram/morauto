...
export default function CustomerEditModal({
  ...
  const [cnhDoc, setCnhDoc] = useState<UploadState>(
    customer.cnhUrl ? { url: customer.cnhUrl, name: "CNH salva" } : null
  )
  const [addrDoc, setAddrDoc] = useState<UploadState>(
    customer.addressProofUrl ? { url: customer.addressProofUrl, name: "Comprovante salvo" } : null
  )
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-zinc-400 text-sm">Documento ({customer.type})</label>
            ...
            {customer.type === "PF" && (
              <>
                <div className="space-y-1">
                  <label className="text-zinc-400 text-sm">Nº CNH</label>
                  <input name="cnh" defaultValue={customer.cnh ?? ""} placeholder="00000000000" className={inp} />
                </div>

                <div className="space-y-1">
                  <label className="text-zinc-400 text-sm">Validade CNH</label>
          ...
          <div className="border-t border-zinc-800 pt-4 space-y-3">
            <p className="text-zinc-400 text-xs font-medium uppercase tracking-wider">Documentos</p>
            ...
            <div className="flex items-center gap-3 bg-zinc-950 border border-zinc-800 rounded-xl p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {cnhDoc ? (
                  <p className="text-emerald-400 text-xs flex items-center gap-1 truncate">
                    <CheckCircle size={11} /> {cnhDoc.name}
                  </p>
                ) : (
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {cnhDoc && (
                  <a href={cnhDoc.url} target="_blank" rel="noopener noreferrer" className="text-zinc-400 hover:text-white text-xs underline">
                    Ver
                  </a>
                )}
                <button
                ...
                >
                  {uploadingCnh ? <Loader2 size={12} className="animate-spin" /> : <Upload size={12} />}
                  {uploadingCnh ? "Enviando..." : cnhDoc ? "Trocar" : "Carregar"}
                </button>
                <input
                  ref={cnhInputRef}
                  type="file"
                  ...
                  onChange={(e) => {
                    ...
                    if (f) uploadFile(f, setUploadingCnh, setCnhDoc)
                  }}
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 bg-zinc-950 border border-zinc-800 rounded-xl p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {addrDoc ? (
                  <p className="text-emerald-400 text-xs flex items-center gap-1 truncate">
                    <CheckCircle size={11} /> {addrDoc.name}
                  </p>
                ) : (
                  <p className="text-zinc-600 text-xs">Nenhum arquivo</p>
                )}
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {addrDoc && (
                  <a href={addrDoc.url} target="_blank" rel="noopener noreferrer" className="text-zinc-400 hover:text-white text-xs underline">
                    Ver
                  </a>
                )}
                <button
                  type="button"
                  disabled={uploadingAddr}
                  onClick={() => addrInputRef.current?.click()}
                  className="flex items-center gap-1.5 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                >
                  {uploadingAddr ? <Loader2 size={12} className="animate-spin" /> : <Upload size={12} />}
                  {uploadingAddr ? "Enviando..." : addrDoc ? "Trocar" : "Carregar"}
                </button>
                <input
                  ref={addrInputRef}
                  type="file"
                  ...
                  onChange={(e) => {
                    const f = e.target.files?.[0]
                    if (f) uploadFile(f, setUploadingAddr, setAddrDoc)
                  }}
                />
              </div>
            </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={isPending || uploadingCnh || uploadingAddr}
